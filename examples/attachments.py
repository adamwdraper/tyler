...
from tyler.storage import initialize_file_store
...
async def setup():
    ...
    # Initialize file store using factory pattern
    await initialize_file_store()
    logger.info("Storage backends initialized")
    
    # Initialize the agent with thread_store and image tools
    agent = Agent(
        model_name="gpt-4o",
    ...
    return thread_store, agent

async def example_manual_attachment(thread_store):
...
async def example_tool_generated_attachment(thread_store, agent):
...
async def example_adding_attachment_to_existing_message(thread_store):
...
async def main():
    # Run examples
    try:
        thread_store, agent = await setup()
        thread1 = await example_manual_attachment(thread_store)
        thread2 = await example_tool_generated_attachment(thread_store, agent)
        thread3 = await example_adding_attachment_to_existing_message(thread_store)
